...
sampled_bi, loss_synthetic = run_decoding(expectiles, taus, np.ones(n_neurons),N=20, minv=min_reward, maxv=max_reward, max_samples=100,max_epochs=5, method='TNC')
sampled_uni, loss_synthetic = run_decoding(expectiles_uni, taus, np.ones(n_neurons),N=20, minv=min_reward, maxv=max_reward, max_samples=100,max_epochs=5, method='TNC')
...
colors_impatience=mpl.cm.viridis(np.linspace(0.1,1,n_neurons))
...
for cue in range(4):
    for neuron in range(n_neurons):
        for i in range(L):
            synthetic_delta_fr_time_uni[cue,neuron, :] += (s[i] ** 2 / (s[i] ** 2 + alpha[cue] ** 2))*(np.dot(U[:, i],FR_uni[cue,neuron,:])*vh[i, :])/s[i]
            if cue==2:
                synthetic_delta_fr_time[neuron, :] += (s[i] ** 2 / (s[i] ** 2 + alpha[cue] ** 2))*(np.dot(U[:, i],FR_synthetic[neuron,:]) * vh[i, :])/s[i]
    ...
    # Correct the range
    synthetic_delta_fr_time_uni[cue, :, max_time] = (synthetic_delta_fr_time_uni[cue, :, max_time] - np.min(synthetic_delta_fr_time_uni[cue, :, max_time])) / (np.max(synthetic_delta_fr_time_uni[cue, :, max_time]) - np.min(synthetic_delta_fr_time_uni[cue, :, max_time]))
    synthetic_delta_fr_time_uni[cue, :, max_time] = synthetic_delta_fr_time_uni[cue, :, max_time] * (np.max(expectiles_uni) - np.min(expectiles_uni)) + np.min(expectiles_uni)
    if cue==2:
        max_time = np.argmax(synthetic_delta_fr_time[neuron, :])
        synthetic_delta_fr_time[:, max_time] = (synthetic_delta_fr_time[:, max_time] - np.min(synthetic_delta_fr_time[:, max_time])) / (np.max(synthetic_delta_fr_time[:, max_time]) - np.min(synthetic_delta_fr_time[:, max_time]))
        synthetic_delta_fr_time[:, max_time] = synthetic_delta_fr_time[:, max_time] * (np.max(expectiles) - np.min(expectiles)) + np.min(expectiles)
        #for neuron in range(n_neurons):
            #ax[cue].plot(time, synthetic_delta_fr_time[neuron, :], color="red")
            #ax[cue].scatter(time, synthetic_delta_fr_time[0, :], color="yellow")

    #for neuron in range(n_neurons):
        #ax[cue].plot(time, synthetic_delta_fr_time_uni[cue, neuron, :], color="blue")
        #ax[cue].scatter(time, synthetic_delta_fr_time_uni[cue, 0, :], color="yellow")
#plt.show()

ax[1].imshow(synthetic_delta_fr_time,extent=[time[0],time[-1],expectiles[-1],expectiles[0]],aspect="auto",cmap="Blues")
...
for cue in range(4):
    for bin_time in range(n_time):
        sampled_dist_synthetic_uni, loss_synthetic = run_decoding(synthetic_delta_fr_time_uni[cue,:, bin_time], taus, np.ones(n_neurons), N=20, minv=min_reward, maxv=max_reward, max_samples=100, max_epochs=5,method='TNC')

        if cue==2:
            sampled_dist_synthetic, loss_synthetic = run_decoding(synthetic_delta_fr_time[:, bin_time], taus,np.ones(n_neurons), N=20, minv=min_reward,maxv=max_reward, max_samples=100, max_epochs=5, method='TNC')
            if np.sum(sampled_dist_synthetic) == 0 or np.sum(sampled_dist_synthetic_uni) == max_reward * n_samples:
                ...
                hist[bin_start:, bin_time] = hist[bin_start:, bin_time] * (1 - np.sum(hist[:bin_start, bin_time])) # Not consider reward =0
        ...
        if np.sum(sampled_dist_synthetic_uni)==0 or np.sum(sampled_dist_synthetic_uni)==max_reward*n_samples:
            ...
            hist_uni[cue,bin_start:,bin_time]=hist_uni[cue,bin_start:, bin_time]*(1-np.sum(hist_uni[cue,:bin_start, bin_time])) # Not consider reward =0

        #for i_s in range(n_samples):
            #all_samples_synthetic.append([time[bin_time],sampled_dist_synthetic[i_s]])
            #all_samples_synthetic_uni.append([time[bin_time],sampled_dist_synthetic_uni[i_s]])
...
for cue in range(3):
    ax.plot_surface(Y, X,(cue)+0*hist_uni[0,bin_start-1:,:], facecolors=scam.to_rgba(hist_uni[cue,bin_start-1:,:]),alpha=None,rstride=1,cstride=1)#,rstride=1,cstride=1,
...
# Variable cue
scam = plt.cm.ScalarMappable(norm=mpl.colors.Normalize(np.min(hist[bin_start-1:,:]), np.max(hist[bin_start-1:,:])),cmap=color_map)
ax.plot_surface(Y, X, (cue+1)+0*hist[bin_start-1:,:], facecolors=scam.to_rgba(hist[bin_start-1:,:]),antialiased = True,rstride=1,cstride=1,alpha=None)
