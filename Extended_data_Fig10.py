...
sampled_bi, loss_synthetic = run_decoding_magnitude(expectiles, taus, np.ones(n_neurons), N=20, minv=min_reward, maxv=max_reward,
                                          max_samples=100, max_epochs=5, method='TNC')
sampled_uni, loss_synthetic = run_decoding_magnitude(expectiles_uni, taus, np.ones(n_neurons), N=20, minv=min_reward,
                                           maxv=max_reward, max_samples=100, max_epochs=5, method='TNC')

gammas = np.linspace(0, 1, n_neurons)
time_reward = [0, 1.5, 3, 6]

X = np.zeros((n_neurons, n_neurons))  # Gammas
Y = np.zeros((n_neurons, n_neurons))  # Expectiles bi
Y_uni = np.zeros((n_neurons, n_neurons))  # Expectiles uni

# Compute synthetic FR at cue
FR_bi = np.zeros((n_neurons, n_neurons))  # Lines correspond to reversal points, columns to temporal discounts
...
for cue in range(4):
    for i_g, gamma in enumerate(gammas):
        for i_r, r in enumerate(expectiles):
            ...
            if cue == 2:
                FR_bi[i_r, i_g] = (gamma ** time_reward[cue]) * expectiles[i_r]

fig, ax = plt.subplots(1, 2, figsize=(2 * horizontal_size, 1 * vertical_size))
ax[0].imshow(FR_bi, extent=[gammas[0], gammas[-1], expectiles[-1], expectiles[0]], aspect="auto", cmap="Blues")
ax[0].set_xlabel(r"$\gamma$")
ax[0].set_ylabel("Reversal point")
ax[0].set_title("Firing Rate at cue")

flat_FR = np.ndarray.flatten(FR_bi)
        ...
        for i in range(L):
            ...
            if cue == 2:
                synthetic_delta_fr_time[neuron, :] += (s[i] ** 2 / (s[i] ** 2 + alpha[cue] ** 2)) * (
                        np.dot(U[:, i], FR_bi[neuron, :]) * vh[i, :]) / s[i]
...
# Upsample to correct for strides in surface plot
