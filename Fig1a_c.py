...
# Value at cue sooner smaller reward = later larger reward
gamma_sel=0.7
r_soon=0.5
...
plt.xlabel("Time")#r"$t$"
plt.ylabel("Value")#r"$V(t)$"
plt.xticks([0],["0"],fontsize=font_size)
plt.yticks([])
#plt.ylim([0,2.5])
#plt.text(0.5,1,"cue")
plt.show()
#fig.savefig(save_dir+r"\value_cartoon.svg")


# Decode future reward using inverse Laplace transform
F=np.zeros((N,N_time))
...
for i_time,time in enumerate(time_reward):
    ...
    for i in range(L):
        ...
        p_stationary+=(s[i]**2)/((s[i]**2)+(alpha**2))*(np.dot(U[:,i],Value_stationary)*vh[i,:]/s[i])
    p[p<0]=0
    p=p/np.sum(p)
...
#plt.plot(times_disc,p_stationary)
plt.show()
#fig.savefig(save_dir+r"\decoded_time_cartoon.svg")


# Multiple temporal discounts value plot
N_neurons=6
#cmap = mpl.cm.magma(np.linspace(0.1,1,N_neurons))
...
plt.yticks([])
plt.xlabel("Reward delay")
plt.ylabel("Responses at cue")#"$V_{\gamma}$"+ "(cue)"
plt.ylim(0,1.1)
#fig.savefig(save_dir+r"\multiple_temporally_discounted_value.svg")
plt.show()
...
plt.yticks([])
#plt.savefig(save_dir+r"\cartoon_joe.svg")
plt.show()
