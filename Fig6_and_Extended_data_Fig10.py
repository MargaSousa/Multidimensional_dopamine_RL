...
# Task/enviornment
task="dusk_dawn"#"sated_hungry""hungry_dusk_dawn"

# Optimal action for each environment
optimal_action=1
# for dusk to dawn it is action 0 or 1

# Plot probablity of selecting optimal action after state change
...
# Trials relative to state change
left=-200
right=2999
...
for i_algo,algo in enumerate(algos):
    ...
    #ax_p_optimal.scatter(i_algo, np.sum(action_after_change_state==1)/action_after_change_state.shape[0],color=colors[i_algo])
    ...
    ax_dynamics.set_ylim([-1,40])
    ax_dynamics.set_yticks([0,40])


    # if task=="hungry_dusk_dawn":
    # if task=="sated_hungry":
    #     ax_dynamics.set_ylim([0, 12])
    # if task=="dusk_dawn":
    #     ax_dynamics.set_ylim([-2, 22])


    ax_dynamics.set_xticks([0,1500,2999],["0","1500","3000"])
    value_algo={}
    ...
    for patch in range(3):
        ...
        # Save
        # for run in range(value.shape[0]):
        #     value_algo["Value patch "+str(patch+1)+" run "+str(run)]=value[run,:,patch]
        # df=pd.DataFrame(value_algo)
        # df.to_csv("Extended_data_Fig_10_"+algo+"_"+task+".csv")


# Save
# df=pd.DataFrame(action_after_change_state_algo)
# df.to_csv("Fig_6_action_after_change_state_task_"+task+".csv")
