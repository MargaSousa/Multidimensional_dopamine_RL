...
mean3 = 3
mean4 = 6
probability = scipy.stats.norm(loc=mean1, scale=scale).pdf(time_init) + scipy.stats.norm(loc=mean2, scale=scale).pdf(
    time_init) + scipy.stats.norm(loc=mean3, scale=scale).pdf(time_init) + scipy.stats.norm(loc=mean4, scale=scale).pdf(
    time_init)
...
probability_remove_short = scipy.stats.norm(loc=mean2, scale=scale).pdf(time_init) + scipy.stats.norm(loc=mean3,
                                                                                                      scale=scale).pdf(
    time_init) + scipy.stats.norm(loc=mean4, scale=scale).pdf(time_init)
...
probability_remove_long = scipy.stats.norm(loc=mean1, scale=scale).pdf(time_init) + scipy.stats.norm(loc=mean2,
                                                                                                     scale=scale).pdf(
    time_init) + scipy.stats.norm(loc=mean3, scale=scale).pdf(time_init)
...
probability_remove_short = scipy.stats.norm(loc=mean2, scale=scale).pdf(time) + scipy.stats.norm(loc=mean3,
                                                                                                 scale=scale).pdf(
    time) + scipy.stats.norm(loc=mean4, scale=scale).pdf(time)
probability_remove_short = probability_remove_short / np.sum(probability_remove_short)

probability_remove_long = scipy.stats.norm(loc=mean1, scale=scale).pdf(time) + scipy.stats.norm(loc=mean2,
                                                                                                scale=scale).pdf(
    time) + scipy.stats.norm(loc=mean3, scale=scale).pdf(time)
probability_remove_long = probability_remove_long / np.sum(probability_remove_long)
...
prob_at_time_scale_remove_short = scipy.stats.norm(loc=mean2, scale=scale).pdf(
    expectiles_remove_short) + scipy.stats.norm(loc=mean3, scale=scale).pdf(expectiles_remove_short) + scipy.stats.norm(
    loc=mean4, scale=scale).pdf(expectiles_remove_short)
...
prob_at_time_scale_remove_long = scipy.stats.norm(loc=mean1, scale=scale).pdf(
    expectiles_remove_long) + scipy.stats.norm(loc=mean2, scale=scale).pdf(expectiles_remove_long) + scipy.stats.norm(
    loc=mean3, scale=scale).pdf(expectiles_remove_long)
...
for n in range(N):
    ax[2].plot(time, gains_after_remove_short[n] * gammas_remove_short[n] ** time, linewidth=linewidth,
               color=color_remove_short, alpha=0.8)
    ax[2].plot(time, gains_after_remove_long[n] * gammas_remove_long[n] ** time, linewidth=linewidth,
               color=color_remove_long, alpha=0.8)
    sum_fr_initial += R * lambdas[n] * np.exp(-lambdas[n] * time)
    fisher_info_initial += R * ((lambdas[n] ** 3) * np.exp(-lambdas[n] * time))
    fisher_info_remove_short += gains_after_remove_short[n] * (gammas_remove_short[n] ** time) * np.log(
        gammas_remove_short[n]) ** 2
    fisher_info_remove_long += gains_after_remove_long[n] * (gammas_remove_long[n] ** time) * np.log(
        gammas_remove_long[n]) ** 2
